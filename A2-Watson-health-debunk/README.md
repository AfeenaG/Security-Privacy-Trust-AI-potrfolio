...
Article & Source TierLead AuditorKey Framework Violations IdentifiedAcademic LiteratureTupasela & Di Nucci (2020)AI & Society (pp. 6, 31)Shivam Patel (p. 13)Fallacy of Inscrutability (Pitfall #18): System obfuscated the criteria used to weight and rank clinical papers (p. 10).
De-emphasised Limitations: Failed to prominent disclosure that training data was strictly sourced from a single US hospital (MSKCC), causing international deployments to collapse due to variant local oncology guidelines (pp. 9, 12).
Mainstream NewsBBC News (2015)"IBM's Watson Supercomputer..." (pp. 6, 31)Jeya Surya Balaji (p. 13)Attributing Agency (Pitfall #1): Used anthropomorphic language ("makes decisions", "learns") (p. 16).
Marketing Echo Chamber (Pitfall #11): Sourced assertions exclusively from IBM executives (e.g., Ginni Rometty) without interviewing independent oncologists, missing failure metrics and baseline benchmarks (pp. 14, 19).Technical PaperIJRCAIT (2024)"Advancing Clinical Decision Support..." (pp. 6, 31)Afeena Gafoor (p. 1)Suggestive Imagery (Pitfall #2): Wrapped algorithmic software inside humanoid robot illustrations to artificially engineer clinician trust (p. 23).Uncertainty Omission (Pitfall #17): Reported static performance metrics (e.g., "97% accuracy") as universal constants while omitting experimental standard deviations, sample size context, or boundaries (p. 24).
...
Use code with caution.If you need to expand this markdown file, let me know if you would like to:Add a summary section breaking down the impact of these pitfalls.Export this data into a different format (like a CSV structure).Cross-reference these specific pitfalls with standard AI governance frameworks.
